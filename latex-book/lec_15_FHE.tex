...
The reason the two conjectures are equivalent are the following. Before
...
ciphertext \(c\) encrypting \(b_1 \oplus \cdots \oplus b_\ell\).
...
a good way to get more familiarity with LWEENC and the kind of
...
linear functions, and even before Gentry's construction people have
managed to achieve homomorphism with respect to slightly larger classes
(e.g., quadratic functions by Boneh, Goh and Nissim) but not
significantly so.
