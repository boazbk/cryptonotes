...
\(\eta\) is a measure (depending on the key) which maps any ciphertext
\(c\) to a number \(\eta(c)\in [0, \infty)\) which we call its
``noise''. For some fixed keypair \((e,d)\) of the scheme, denote
\begin{equation*}
...
\begin{itemize}
\item
  \(E_e(b)\in \mathcal{C}_b^{\sqrt{q}}\) for any plaintext \(b\). That
  is, ``fresh encryptions'' have noise at most \(\sqrt{q}\).
\item
  If \(c\in\mathcal{C}_b^\eta\) with \(\eta\leq q/4\), then
  \(D_d(c)=b\). That is, as long as the noise is at most \(q/4\) (which
  is \(\gg \sqrt{q}\)), decryption will still succeed.
\item
  ...
  \end{equation*}
  as long as \(n^3\cdot \max\{\eta,\eta'\}<q/4\). That is, as long as
  noise is not too large, applying \(\ensuremath{\mathit{ENAND}}\) to
  \(c\) and \(c'\) will yield an encryption of
  \(\ensuremath{\mathit{NAND}}(D_d(c),D_d(c'))\) with noise level that
  is not ``too much higher'' than the maximum noise of \(c\) and \(c'\).
\end{itemize}
...
\begin{figure}
\centering
\includegraphics[width=\textwidth, height=0.25\paperheight, keepaspectratio]{../figure/fhe-noisy.png}
\caption{In a noisy homomorphic encryption, every ciphertext \(c\) has a
``noise'' parameter \(\eta(c)\) associated with it. When we encrypt
\(0\) or \(1\), we get a ciphertext with noise at most \(\sqrt{q}\),
while we are guaranteed to successfully decrypt. Applying the
\(\ensuremath{\mathit{ENAND}}\) operation to two ciphertexts \(c\) and
\(c'\) yields a ciphertext with noise level at most \(n^3\) times the
maximum noise of \(c\) and \(c'\). Hence we can compose
\(\ensuremath{\mathit{ENAND}}\) operations to apply any NAND circuit of
depth at most \(\ell\) to fresh encryptions, and succeed in obtaining a
ciphertext decrypting to the circuit output as long as
\(n^{3\ell}\sqrt{q} \ll q/4\).}
\label{noisefhefig}
\end{figure}
...
We will assume the LWE conjecture with \(q(n) \approx 2^{\sqrt{n}}\) in
...
\begin{pause} \label[pause]{15-Please-try-to-verify-t}

Please try to verify that if \(C,C'\) are encryptions of \(b,b'\) then
\(C \oplus C'\) and \(C \otimes C'\) will be encryptions of \(b+b'\) and
\(bb'\) respectively.
...
\begin{quote} \label[quote]{15-FHEENCKey-generation-A}
...
\def\labelenumi{\arabic{enumi}.}
\item
  For every \(\hat{s}\in \{0,1\}^n\) such that
  \(|\langle \hat{s} \rangle,c|<0.1q\), \(f(\hat{s})=0\)
\item
  For every \(\hat{s}\in \{0,1\}^n\) such that
  \(0.4q<|\langle \hat{s} \rangle,c|<0.6q\), \(f(\hat{s})=1\)
\item
  There is a circuit computing \(f\) of depth at most \(100(\log m)^3\).
\end{enumerate}

\end{lemma}

::: \{.proof data-ref=``decdepthlem''\} For every number \(x\in\Z_q\),
...
\hypertarget{compFHE}{}
\begin{definition}[Compressible Fully Homomorphic Encryption] \label[definition]{compFHE}

A \emph{compressible fully homomorphic public key encryption scheme} is
a CPA secure public key encryption scheme \((G,E,D)\) such that there
exist polynomial-time algorithms
...
\hypertarget{ratecompFHE}{}
\begin{definition}[Rate of Compressible Fully Homomorphic Encryption] \label[definition]{ratecompFHE}

A compressible fully homomorphic public key encryption scheme has
\emph{rate} \(\alpha=\alpha(n)\) if for every \((e,d)=G(1^n)\),
\(\ell=poly(n)\), \(x_1,\ldots,x_\ell \in \{0,1\}\), and
\(f:\{0,1\}^\ell\rightarrow \{0,1\}^*\) with sufficiently long output,
it holds that
\begin{equation*}
...
\hypertarget{optrate}{}
\begin{theorem}[Nearly Optimal Rate] \label[theorem]{optrate}
