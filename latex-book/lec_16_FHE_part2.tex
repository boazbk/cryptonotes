...
Construction}\label{Fully-homomorphic-encrypt}
...
\begin{pause} \label[pause]{I-recommend-you-pause-her}
...
\[(C\otimes C')v = \widehat{(\ensuremath{\mathit{CQ}}^\top)}C'v = \widehat{(\ensuremath{\mathit{CQ}}^\top)}(bv+e') \;. \label{fhemultfinaleqfirst}\]
...
\begin{proof} \label[proof]{For-starters-let-us-see-t}
...
every \(i\) and \(I\) is the \((n\log q)\times (n\log q)\).
...
\begin{proof} \label[proof]{This-follows-from-the-cal}
...
\subsection{Shallow decryption circuit}\label{Shallow-decryption-circui}
...
\(\hat{q}\) with \(\ensuremath{\mathit{CQ}}^\top Q\)) and outputs \(0\)
...
\begin{proof} \label[proof]{For-every-number-xinZq-wr}
...
\begin{pause} \label[pause]{Now-would-be-a-good-point}
...
\section{Example application: Private information
retrieval}\label{Example-application-Priva}
