...
\(\Pr[ Eve(E_k(m))=1] = \Pr[ Eve(E_k(m'))=1]\) since these correspond
...
encryption of \(m\) if \(c=c^*\) and otherwise will toss a coin will
have some advantage over \(1/2\) in distinguishing an encryption of
\(x\) from an encryption of \(m'\).
...
\item
  ...
  distributions \(\{ E_k(m_0) \}_{k \sim \{0,1\}^n}\) and
  \(\{ E_k(m_1) \}_{k \sim \{0,1\}^n}\) are identical.
\item
...
where we denote
\(\max(\mathcal{D}) = \max_{m^*\in \{0,1\}^{\ell(n)}} \Pr_{m \sim \mathcal{D}}[m=m^*]\)
...
\(D_k(c)= v \oplus k\). By the associative law of addition (which works
also modulo two),
...
obtained by picking \(k \sim \{0,1\}^n\) and outputting \(E_k(x)\).
