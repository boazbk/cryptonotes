...
The actual encryption WEP used was RC4, but for us it doesn't really
...
\begin{figure}
\centering
\includegraphics[width=\textwidth, height=0.25\paperheight, keepaspectratio]{../figure/cca-game.jpg}
...
\begin{theorem}[CCA from CPA and MAC (encrypt-then-sign)] \label[theorem]{CCAfromCPAMACthm}
...
exercises}\label{6-Reading-comprehension-}
...
\begin{exercise} \label[exercise]{6-Let-ED-be-the-canonica}

Let \((E,D)\) be the ``canonical'' PRF-based CPA secure encryption,
where \(E_k(m)= (r,f_k(r)\oplus m)\) and \(\{ f_k \}\) is a PRF
collection and \(r\) is chosen at random. Is this scheme CCA secure?

\begin{enumerate}
\def\labelenumi{\alph{enumi}.}
\item
  No it is never CCA secure.
\item
  It is always CCA secure.
\item
  It is sometimes CCA secure and sometimes not, depending on the
  properties of the PRF \(\{ f_k \}\).
\end{enumerate}

\end{exercise}

\begin{exercise} \label[exercise]{6-Suppose-that-we-allow-}

Suppose that we allow a key to be as long as the message, and so we can
use the one time pad. Would the one-time pad be:
...
\item
  CCA secure
\item
  Neither CPA nor CCA secure.
\end{enumerate}

\end{exercise}

\begin{exercise} \label[exercise]{6-Which-of-the-following}

Which of the following statements is true about the proof of
\cref{CCAfromCPAMACthm}:

\begin{enumerate}
\def\labelenumi{\alph{enumi}.}
\item
  Case I corresponds to breaking the MAC and Case II corresponds to
  breaking the CPA security of the underlying encryption scheme.
\item
  Case I corresponds to breaking the CPA security of the underlying
  encryption scheme and Case II corresponds to breaking the MAC.
\item
  Both cases correspond to both breaking the MAC and encryption scheme
\item
  If neither Case I nor Case II happens then we obtain an adversary
