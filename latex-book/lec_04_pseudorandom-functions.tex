...
\(s\in\{0,1\}^*\) and \(i \in \{0,\ldots,2^{|s|}-1\}\) and outputting a
...
\begin{definition}[PRF ensembles] \label[definition]{prfensemnledef}

Let \(\{ f_s \}_{s\in \{0,1\}^*}\) be an ensemble of functions such that
for every \(s\in \{0,1\}^*\), \(f_s:\{0,1\}^{|s|} \rightarrow \{0,1\}\).
We say that \(\{ f_s \}\) is a \emph{pseudorandom function ensemble} if
the function \(F\) that on input \(s\in \{0,1\}^*\) and
\(i \in \{0,\ldots,2^{|s|}-1\}\) outputs \(f_s(i)\) is a PRF generator.

\end{definition}

Note that the condition of \cref{prfensemnledef} corresponds to
...
\begin{pause} \label[pause]{4-It-is-worth-while-to-p}

It is worth while to pause and make sure you understand why
\cref{prfensemnledef} and \cref{prfdef} give different ways to talk
...
will then output \(1\) if and only if \(M\) succeeds in impersonation in
...
\subsection{Modifying input and output lengths of
PRFs}\label{4-Modifying-input-and-ou}
...
functions have a longer output \(\ell\). Specifically, we can make the
following definition:

\hypertarget{prfensembledinputdef}{}
\begin{definition}[PRF ensemble (varying inputs and outputs)] \label[definition]{prfensembledinputdef}

Let \(\ell_{\text{in}},\ell_{\text{out}}:\N \rightarrow \N\). An
ensemble of functions \(\{ f_s \}_{s\in \{0,1\}^*}\) is a \emph{PRF
ensemble with input length \(\ell_{\text{in}}\) and output length
\(\ell_{\text{out}}\)} if:

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  For every \(n\in\N\) and \(s \in \{0,1\}^n\),
  \(f_s:\{0,1\}^{\ell_{\text{in}}} \rightarrow \{0,1\}^{\ell_{\text{out}}}\).
\item
  For every polynomial \(p\) and \(p(n)\)-time adversary \(A\), if \(n\)
...
Standard PRFs as we defined in \cref{prfensemnledef} correspond to
generalized PRFs where \(\ell_{\text{in}}(n)=n\) and
\(\ell_{\text{out}}(n)=1\) for all \(n\in\N\). It is a good exercise
(which we will leave to the reader) to prove the following theorem:

\hypertarget{PRFlengthextensionthm}{}
\begin{theorem}[PRF length extension] \label[theorem]{PRFlengthextensionthm}

Suppose that PRFs exist. Then for every constant \(c\) and
polynomial-time computable functions
\(\ell_{\text{in}},\ell_{\text{out}}:\N \rightarrow \N\) with
\(\ell_{\text{in}}(n), \ell_{\text{out}}(n) \leq n^c\), there exist a
PRF ensemble with input length \(\ell_{\text{in}}\) and output length
\(\ell_{\text{out}}\).

\end{theorem}

Thus from now on whenever we are given a PRF, we will allow ourselves to
...
\item
  ...
  \(V_k(m',\tau')=1\).\footnote{Clearly if the adversary outputs a pair
    ...
    in the past. As above, one can thwart this by insisting the every
...
\(n/2\) bits output (which we can obtain using
\cref{PRFlengthextensionthm}). We define \(S_k(m) = F(k,m)\) and
\(V_k(m,\tau)\) to output \(1\) iff \(F(k,m)=\tau\). Suppose towards the
sake of contradiction that there exists an adversary \(A\) breaks the
security of this construction of a MAC. That is, \(A\) queries
\(S_k(\cdot)\) \(poly(n)\) many times and with probability \(1/p(n)\)
for some polynomial \(p\) outputs \((m',\tau')\) that she did \emph{not}
ask for such that \(F(k,m')=\tau'\).
...
\(2^{-n/2}\). If instead \(O(\cdot) = F(k,\cdot)\), then by the fact
...
\(F(k,\cdot)\) and an oracle to a random function \(H\), which gives us
a contradiction.

\end{proof}

\section{Arbitrary input length extension for MACs and
