...
for \(x\) a short (i.e., \(n\) bit long) random string, often known as
the \emph{seed} of the pseudorandom generator, and a truly random long
(i.e., of length \(\ell(n)\)) string chosen uniformly at random from
\(\{0,1\}^{\ell(n)}\).
...
\emph{pseudorandom generator} if \(G(x)\) for a random short
...
distribution where the first \(i\) bits chosen uniformly at random,
whereas the last \(t-i\) bits are computed as above. Namely, we choose
\(s_i\) at random in \(\{0,1\}^n\) and continue the computation of
...
some non-negligible \(\epsilon\). From \(Eve\), we will design an
...
\(G(U_n)\) while the last \(\ell-i\) bits are all random. Notice that
\(H_0=U_\ell\) and \(H_\ell=G(U_n)\), so it suffices to show that
\(H_{i-1} \approx H_{i}\) for all \(i\).

Suppose \(H_{i-1} \not\approx H_{i}\) for some \(i\), i.e.~there exists
some \(Eve\) and non-negligible \(\epsilon\) such that
\begin{equation*}
\Pr[Eve(H_{i})=1]-\Pr[Eve(H_{i-1})=1]>\epsilon(n).
\end{equation*}
Consider the program \(P\) that, on input \((y_1,\ldots,y_{i-1})\),
picks the bits \(\hat y_{i},\ldots, \hat y_\ell\) uniformly at random.
Then, \(P\) calls \(Eve\) on the generated input. If \(Eve\) outputs
\(1\) then \(P\) outputs \(\hat y_{i}\), and otherwise it outputs
\(1-\hat y_{i}\).
...
\(Eve(y)=Eve'(U_{n+1}\oplus m)\). Then, \(Eve\) can distinguish the two
...
\item
  Bob picks \(s\leftarrow_R\{0,1\}^n\) and \(b \leftarrow_R \{0,1\}\).
  If \(b=0\) then Bob sends \(y=G(s)\) and if \(b=1\) he sends
  \(y=G(s)\oplus z\). In other words, \(y = G(s) \oplus b\cdot z\) where
  \(b\cdot z\) is the vector \((b\cdot z_1,\ldots, b\cdot z_{3n})\).
\item
  Alice then picks a random \(b'\leftarrow_R\{0,1\}\) and sends it to
  Bob.\\
\item
  Bob sends to Alice the string \(s\) and \(b\). Alice verifies that
  indeed \(y= G(s) \oplus b \cdot z\). Otherwise Alice aborts.
\item
...
prove it is an excellent exercise. Two main components in the proofs
are:

\begin{itemize}
\item
  With probability \(1-negl(n)\) over \(z \leftarrow_R \{0,1\}^{3n}\),
  the sets \(S_0 = \{ G(x) | x\in \{0,1\}^n \}\) and
  \(S_1 = \{ G(x) \oplus z | x\in \{0,1\}^n \}\) will be disjoint. Hence
  by choosing \(z\) at random, Alice can ensure that Bob is
  \emph{committed} to the choice of \(b\) after sending \(y\).
\item
  For every \(z\), both the distribution \(G(U_n)\) and
  \(G(U_n)\oplus z\) are pseudorandom. This can be shown to imply that
  no matter what string \(z\) Alice chooses, she cannot predict \(b\)
  from the string \(y\) sent by Bob with probability better than
  \(1/2 + negl(n)\). Hence her choice of \(b'\) will be essentially
  independent of \(b\).
\end{itemize}
...
We define \(\ensuremath{\mathit{QR}}_N\) to be the set of
...
The number theory required to show takes a while to develop. However, it
is interesting and I recommend the reader to search up this particular
generator, see for example
\href{https://www.cs.miami.edu/home/burt/learning/Csc609.062/docs/bbs.pdf}{this
survey by Junod}.
