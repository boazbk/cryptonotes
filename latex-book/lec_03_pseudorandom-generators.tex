...
\begin{pause} \label[pause]{This-definition-as-is-often-th}
...
\begin{proof} \label[proof]{On-input-yinn-consider-the-alg}
...
\begin{proof} \label[proof]{The-proof-of-this-theorem-is-v}
...
\begin{proof} \label[proof]{For-the-forward-direction-supp}
...
succeeds,\[\left| \Pr[Eve(G(U_n))=1] - \Pr[ Eve(U_\ell)=1] \right|=|p-\frac12|\ge \epsilon(n),\]
a contradiction.
...
\[\Pr[Eve(H_{i})=1]-\Pr[Eve(H_{i-1})=1]>\epsilon(n).\] Consider the
program \(P\) that, on input \((y_1,\ldots,y_{i-1})\), picks the bit
\(\hat y_{i},\ldots, \hat y_n\) uniformly at random. Then, \(P\) calls
\(Eve\) on the generated input. If \(Eve\) outputs \(1\) then \(P\)
outputs \(\hat y_{i}\), and otherwise it outputs \(1-\hat y_{i}\).
...
\begin{proof} \label[proof]{Recall-that-the-one-time-pad-i}
...
\begin{pause} \label[pause]{You-should-really-pause-here-a}
...
bits}\label{Attempt--Linear-Congruential-G}
...
Generator}\label{Case-Study--Subset-Sum-Generat}
...
\begin{pause} \label[pause]{This-is-an-excellent-point-for}
...
\section{Case Study 3: B.B.S.}\label{Case-Study--BBS}
...
\begin{proofidea} \label[proofidea]{The-proof-uses-an-extremely-us}
...
\begin{proof}[Proof of \cref{prgexist}] \label[proof]{Let-epsilonTellm-be-as-in-the-}
