...
\begin{pause} \label[pause]{8-A-natural-approach-is-}
...
\begin{proof} \label[proof]{8-The-certificate-pi-con}
...
\section{Proofs of Retrievability}\label{8-Proofs-of-Retrievabili}
...
\(h_s(X)\) is computationally indistinguishable from the uniform
...
entropy (with some extra ``security margin'') then the output \(h_s(X)\)
...
\begin{proof} \label[proof]{8-Let-h-be-chosen-as-abo}
...
samples from \(Y_s\) are identical. Then with probability at least
...
secrecy}\label{8-Forward-and-backward-s}
