\chapter{Public key cryptography}\label{9-Public-key-cryptograph}
...
\caption{Ralph Merkle's Berkeley CS 244 project proposal for developing
...
\section{Private key crypto recap}\label{9-Private-key-crypto-rec}
...
Definition}\label{9-Public-Key-Encryptions}
...
\begin{pause} \label[pause]{9-Despite-it-being-a-cho}
...
\subsection{The obfuscation paradigm}\label{9-The-obfuscation-paradi}
...
\begin{quote} \label[quote]{9-Obfuscation-based-publ}
...
\section{Some concrete candidates:}\label{9-Some-concrete-candidat}
...
El-Gamal)}\label{9-Diffie-Hellman-Encrypt}
...
\begin{proof} \label[proof]{9-Using-a-discrete-logar}
...
we can prove that this protocol is secure in the random-oracle model,
under the assumption that the task of computing \(g^{ab}\) from \(g^a\)
and \(g^b\) (which is now known as the \emph{Diffie-Hellman problem}) is
hard.
...
\begin{pause} \label[pause]{9-Please-take-your-time-}
...
\begin{proof} \label[proof]{9-For-CPA-security-we-ne}
...
\((x,y,z)\in \Z_p^3\) that satisfy a certain equation, where
multiplication can be defined in a certain way. The main advantage of
elliptic curve cryptography is that the best known algorithms run in
time \(2^{\approx n}\) as opposed to \(2^{\approx n^{1/3}}\), which
allows for much shorter keys. Unfortunately, elliptic curve cryptography
is just as susceptible to quantum algorithms as the discrete logarithm
problem over \(\Z_p\).

\end{remark}

\hypertarget{DHKErem}{}
\begin{remark}[Encryption vs Key Exchange and ElGamal] \label[remark]{DHKErem}
...
\subsection{Sampling random primes}\label{9-Sampling-random-primes}
...
\begin{proof} \label[proof]{9-Recall-that-the-least-}
...
\(2^{N-1} \leq N^k\), and taking logs we get that \(N-1 \leq k \log N\)
or \(k \geq (N-1)/\log N\). (We can assume that \(N\) is odd without of
...
\(C_0,\ldots,C_{N-1}\). Since \(\int_0^1 x^k dx = \tfrac{1}{k+1}\), we
see that \(I\) is a sum of fractions with integer numerators and with
...
theory.}\label{9-A-little-bit-of-group-}
...
\item
  ...
  \(a\cdot b = b\cdot a\) and \((a\cdot b)\cdot c = a\cdot (b\cdot c)\).
\item
...
\begin{pause} \label[pause]{9-Try-to-stop-here-and-v}
...
\subsection{Digital Signatures}\label{9-Digital-Signatures}
...
(DSA)}\label{9-The-Digital-Signature-}
...
\begin{pause} \label[pause]{9-You-should-pause-here-}
...
\(\sigma\) does not reveal information about \(b\) and \(a\) because
this is ``masked'' by the ``random'' value \(H(m)\). On the other hand,
if an adversary is able to come up with valid signatures, then at least
if we treated \(H\) and \(F\) as oracles, if the signature passes
verification then (by taking \(\log\) to the base of \(g\)) the answers
\(x,y\) of these oracles will satisfy \(b\sigma = x + ay\), which means
that sufficiently many such equations should be enough to recover the
discrete log \(a\).

\begin{pause} \label[pause]{9-Before-seeing-the-actu}
...
\begin{proof} \label[proof]{9-Suppose-for-the-sake-o}
...
output \((f,\sigma)\). We will then record the value \(F(f)=r\) and
answer \(r\) on future queries to \(F\). If we've already answered
\(F(f)\) before with a different value, then we halt the experiment and
output an error. We claim that the adversary's chance of succeeding in
...
\(b=\log_g f \mod p\) is uniform as well, and so the distribution of the
signature \((f,\sigma)\) is identical to the distribution by a real box.
...
practice.}\label{9-Putting-everything-tog}
...
\texttt{https://} URLs. The security this achieves is quite amazing. No
...
present terminate the interaction or at least display some warning.
Clearly a person in the middle can stop this message from travelling and
hence not allow the interaction to continue, but they cannot
\emph{spoof} the message and send a certificate for their own public
key, unless they know Apple's secret key. (In today's actual
...
\caption{When you connect to a webpage protected by SSL/TLS, the browser
...
Spain, Autoritat de Certificación de la Comunitat Valenciana (ACCV),
...
primes}\label{9-Appendix-An-alternativ}
...
\begin{proof} \label[proof]{9-Let-Nn-We-need-to-show}
...
Proofs}\label{9-Additional-Group-Theor}
...
\tightlist
\item
  Exponentiation: \(a,b\mapsto a^b\). This is done efficiently using
  repeated squaring, i.e.~generate all the squares up to \(2^k\) and
  then use the binary representation.
\item
  Inverse: \(a \mapsto a^{-1}\). This is done efficiently in
  \(\mathbb{Z}_p^{\*}\) by Fermat's little theorem. \(a^{-1}=a^{p-2}\)
  mod \(p\).
\end{itemize}

\subsection{Solved exercises:}\label{9-Solved-exercises}
...
\begin{solution} \label[solution]{9-Yes-if-multiplication-}
...
\begin{solution} \label[solution]{9--and--Recall-that-a-ge}
...
\begin{solution} \label[solution]{9-The-orders-of--are----}
...
\begin{solution} \label[solution]{9-Suppose-that-a-in-math}
...
\begin{solution} \label[solution]{9-Suppose-that-ca--c-for}
...
\begin{solution} \label[solution]{9-To-be-a-subgroup-we-ne}
...
\begin{solution} \label[solution]{9-Suppose-that-we-receiv}
