...
Definition}\label{Public-Key-Encryptions-Definit}
...
\begin{pause} \label[pause]{Despite-it-being-a-chosen-plai}
...
\begin{proof} \label[proof]{Using-a-discrete-logarithm-alg}
...
hard.\footnote{One can get security results for this protocol without a
  ...
  \emph{Decisional Diffie-Hellman (DDH)} assumption: for a random
  \(a,b, u \in \mathbb{Z}_p\) (prime \(p\)), the triple
  \((g^a, g^b, g^{ab})\approx (g^a, g^b, g^u)\). This implies CDH (can
  you see why?). DDH also restricts our focus to groups of prime order.
  In particular, DDH does not hold in even-order groups. For example,
  DDH does not hold in \(\mathbb{Z}^{\*}_p=\{1,2\ldots p-1\}\) (with
  group operation multiplication mod \(p\)) since half of its elements
  are quadratic residues \emph{and} it is efficient to test if an
  element is a quadratic residue using Fermat's little theorem (can you
  see why? See Exercise 10.7). However, DDH holds in subgroups of
  \(\mathbb{Z}^{\*}_p\) of prime order. If \(p\) is a safe prime
  (i.e.~\(p=2q+1\) for a prime \(q\)), then we can instead use the
  subgroup of quadratic residues, which has prime order \(q\). See
  Boneh-Shoup 10.4.1 for more details on the underlying groups for CDH
  and DDH.}
...
\begin{pause} \label[pause]{Please-take-your-time-to-re-re}
...
\begin{proof} \label[proof]{For-CPA-security-we-need-to-pr}
...
\begin{proof} \label[proof]{Recall-that-the-least-common-m}
...
\begin{pause} \label[pause]{Try-to-stop-here-and-verify-al}
...
sometimes consider the notion of \emph{strong unforgability} which
...
security into a signature that satisfies this stronger unforgability
condition.
...
\begin{pause} \label[pause]{You-should-pause-here-and-veri}
...
\begin{pause} \label[pause]{Before-seeing-the-actual-proof}
...
CerticÃ¡mara S.A., Certigna, Certinomis, certSIGN, China Financial
...
AdministraciÃ³ Oberta de Catalunya (Consorci AOC, CATCert), Cybertrust
...
Spain, Autoritat de CertificaciÃ³ de la Comunitat Valenciana (ACCV),
...
Microsec e-SzignÃ³ CA, NetLock Ltd., PROCERT, QuoVadis, RSA the Security
...
primes}\label{Appendix-An-alternative-proof-}
...
\begin{proof} \label[proof]{Let-Nn-We-need-to-show-that-th}
...
Proofs}\label{Additional-Group-Theory-Exerci}
...
\begin{solution} \label[solution]{Yes-if-multiplication-and-no-i}
...
\begin{solution} \label[solution]{-and--Recall-that-a-generator-}
...
\begin{solution} \label[solution]{The-orders-of--are------respec}
...
\begin{solution} \label[solution]{Suppose-that-a-in-mathbbG-and-}

Suppose that \(a \in \mathbb{G}\) and that \(b,c \in \mathbb{G}\) such
that \(ab = 1\) and \(ac = 1\). Then we know that \(ab = ac\), and then
we can apply \(a^{-1}\) to both sides (we are guarunteed that \(a\) has
...
\begin{solution} \label[solution]{Suppose-that-ca--c-for-all-c-i}
...
\begin{solution} \label[solution]{To-be-a-subgroup-we-need-to-ma}
...
\begin{solution} \label[solution]{Suppose-that-we-recieve-some-e}
