...
\begin{quote}
\textbf{Scheme LwoE-ENC:} Public key encryption under the hardness of
...
\begin{quote}
\textbf{LWE-based encryption LWEENC:}
...
Indeed by \cref{LWEsearchtodecthm} (search to decision reduction) this
conjecture implies that the distribution \(X\) over pairs \((A,y)\) with
\(y=Ax+e\) is indistinguishable from the distribution \(X'\) over pairs
\((A,y')\) where \(y'\) is uniform. But if there was some
polynomial-time algorithm \(T\) distinguishing \(D\) from \(D'\) then we
can design a randomized polynomial-time algorithm \(T'\) distinguishing
\(X\) from \(X'\) with the same advantage by setting
\(T'(A,y)=T(A,y,w^\top A,\langle w,y \rangle)\) for random
\(w \leftarrow_R \{0,1\}^m\).
...
we mentioned before and is also ``Wikipedia-able'') but the idea is
simple. For every \(m\times (n+1)\) matrix \(A'\) over \(\Z_q\), define
\(h_{A'}:\Z_q^m \rightarrow \Z_q^n\) to be the map
