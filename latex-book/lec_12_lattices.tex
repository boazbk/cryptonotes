\chapter{Lattice based cryptography}\label{11-Lattice-based-cryptogr}
...
started several years ago). Based on this, the National Institute of
Standards and Technology has started a
\href{https://csrc.nist.gov/projects/post-quantum-cryptography}{process
to identify ``post quantum'' public key encryption scheme}. All the
finalist for public-key encryption are based on lattices/codes.
...
\subsection{Quick linear algebra recap}\label{11-Quick-linear-algebra-r}
...
You should be comfortable with the following notions (these are covered
in a number of sources, including the appendix of Katz-Lindell and
\href{https://shoup.net/ntb/}{Shoup's online-available book}):
...
\item
  ...
  \(n\times 1\) matrix. We write the \(i\)-th coordinate of \(v\) as
...
\item
  ...
  \(n\) is defined as \(\sum u_i v_i\) (addition done in the field
  \(\mathbb{F}\)).\footnote{Inner products can be defined more
    generally, and in particular over fields such as the complex numbers
    we would define the inner product as \(\sum \overline{u}_i v_i\)
    where for \(a\in \mathbb{C}\), \(\overline{a}\) denotes the
    \emph{complex conjugate} of \(a\). However, we stick to this simple
    case for this chapter.}
\item
  The \emph{matrix product} \(\ensuremath{\mathit{AB}}\) of an
  \(m \times k\) and a \(k\times n\) matrix results in an \(m\times n\)
  matrix. If we think of the rows of \(A\) as the vectors
  \(A_1,\ldots,A_m \in \mathbb{F}^k\) and the columns of \(B\) as
  \(B_1,\ldots,B_n \in \mathbb{F}^k\), then the \((i,j)\)-th coordinate
  of \(\ensuremath{\mathit{AB}}\) is \(\langle A_i , B_j \rangle\).
  Matrix product is associative and satisfies the distributive law but
  is \emph{not commutative}: there are pairs of square matrices \(A,B\)
...
\item
  ...
  \(m \geq n\)) then by dropping \(m-n\) rows of \(A\) and coordinates
  of \(y\) we can obtain the equation \(A'x = y'\) where \(A'\) is an
  \(n\times n\) matrix that has an inverse. In this case a solution (if
  it exists) will be equal to \((A')^{-1}y\). If for a set of equations
  we have \(m>n\) and we can find two such matrices \(A',A''\) such that
  \((A')^{-1}y \neq (A'')^{-1}y\) then we say it is \emph{over
  determined} and in such a case it has no solutions. If a set of
  equations has more variables \(n\) than equations \(m\) we say it's
  \emph{under-determined}. In such a case it either has no solutions or
  the solutions form an affine subspace of dimension at least \(n-m\).
\item
...
elimination}\label{11-A-world-without-Gaussi}

The general approach people use to get a public key encryption is to
...
Pretend that we didn't know of Gaussian elimination,\footnote{Despite
  ...
  in the west through the 1670 notes of Isaac Newton, more than 100
  years before Gauss was born.} and that if we picked a ``generic''
matrix \(A\) then the map \(x \mapsto Ax\) would be hard to invert.
(Here and elsewhere, our default interpretation of a vector \(x\) is as
a \emph{column} vector, and hence if \(x\) is \(n\) dimensional and
\(A\) is \(m\times n\) then \(Ax\) is \(m\) dimensional. We use
\(x^\top\) to denote the row vector obtained by \emph{transposing}
\(x\).) Could we use that to get a public key encryption scheme?
...
\begin{pause} \label[pause]{11-If-you-have-a-CPA-secu}
...
and only if the equation is incorrect.
...
\begin{quote} \label[quote]{11-Scheme-LwoE-ENC-Public}
...
\begin{pause} \label[pause]{11-Please-stop-here-and-m}
...
\section{Security in the real world.}\label{11-Security-in-the-real-w}
...
with this issue. However, from the cryptographic point of view, these
errors can be our saving grace and enable us to salvage the security of
the ridiculous scheme above.
...
\begin{quote} \label[quote]{11-Conjecture-Learning-wi}
...
\end{equation*}
where for \(q=q(n)\) and \(\delta = \delta(n)\), this probability is
taken over \(A\) a random \(m\times n\) matrix over \(\Z_q\), \(x\) a
random vector in \(\Z_q^n,\) and \(e\) a random ``noise vector'' in
\(\Z_q^m\) where \(|e_i| < \delta q\) for every
\(i \in [m]\).\footnote{One can think of \(e\) as chosen by simply
  letting every coordinate be chosen at random in
  \(\{ -\delta q, -\delta q + 1 , \ldots, +\delta q \}\). For technical
  reasons, we sometimes consider other distributions and in particular
  the \emph{discrete Gaussian} distribution which is obtained by letting
  ...
  integer. (A closely related distribution is obtained by picking such a
  Gaussian random variable and then rounding it to the nearest integer.)}
...
\begin{pause} \label[pause]{11-The-LWE-conjecture-pos}

The LWE conjecture posits that no \emph{efficient} algorithm can recover
\(x\) given \(A\) and \(Ax + e\). But you might wonder whether it's
possible to do this is \emph{inefficiently}. The answer is yes.
Intuitively the reason is that if we have more equations than unknown
(i.e., if \(m>n\)) then these equations contain enough information to
determine the unknown variables even if they are noisy. It can be shown
that if \(m\) is sufficiently large (\(m>10n\) will do) then with high
probability over \(A,x,e\), given \(A\) and \(y=x +e\), if we enumerate
over all \(\tilde{x} \in \Z_q^n\) and output the string minimizing
\(| A\tilde{x} - y|\) (where we define \(|v| =\sum |v_i|\) for a vector
\(v\)), then \(\tilde{x}\) will equal \(x\).

It is a good exercise to work out the details, but a hint is this can be
proven by showing that for every \(\tilde{x}\neq x\), with high
probability over \(A\), \(|A\tilde{x} - Ax| > \delta q m\). The latter
fact holds because \(v = A(x-\tilde{x})\) is a random vector in
\(\Z_q^m\), and the probability that \(|v|<\delta q m\) is much smaller
than \(q^{-0.1 m} < q^{-n}\). Hence we can take a union bound over all
possible \(\tilde{x} \in \Z_q^n\).

\end{pause}

\section{Search to decision}\label{11-Search-to-decision}
...
\begin{figure}
\centering
\includegraphics[width=\textwidth, height=0.25\paperheight, keepaspectratio]{../figure/lwevecindist.png}
...
\begin{proof} \label[proof]{11-Suppose-that-we-had-a-}
...
well. Now note that \(Ax + (r\|0^m\cdots \|0^m)x = Ax + x_1 r\) and
hence if \(x_1 = a\) then we still have an input of the same form
\((A',A'x+e)\).
...
adding a random and independent vector \(r\) to \(y'\), which means that
the distribution \((A',y')\) is uniform and independent.
...
\begin{quote} \label[quote]{11-LWE-based-encryption-L}

\textbf{LWE-based encryption LWE-ENC:}

\begin{itemize}
\item
  \emph{Parameters:} Let \(\delta(n)=1/n^4\) and let \(q=poly(n)\) be a
  prime such that LWE holds w.r.t. \(q,\delta\). We let
  \(m = n^2\log q\).
\item
...
\begin{pause} \label[pause]{11-The-scheme-LWEENC-is-a}
...
\begin{figure}
\centering
\includegraphics[width=\textwidth, height=0.25\paperheight, keepaspectratio]{../figure/lweencdesc.png}
...
\begin{proof} \label[proof]{11-langle-wtop-Ax-rangle-}
...
\item
  ...
  \(\Z_q^{m\times n}\), \(x\) is uniform in \(\Z_q^n\), \(e \in \Z_q^m\)
  is chosen with \(e_i \in \{-\delta q,\ldots,+\delta q\}\), \(y=Ax+e\),
...
\begin{pause} \label[pause]{11-You-should-stop-here-a}
...
\begin{proof}[Proof of \cref{LWEENClem}] \label[proof]{11-Define-D-to-be-the-dis}
...
\(m\times n+1\) matrix over \(\Z_q\), then with probability at least
...
The claim completes the proof of the lemma, since letting \(A'\) be the
matrix \((A|y)\) and \(z=(a,\sigma)\), we see that the distribution
...
\textbf{Proof of claim:} The proof of this claim relies on the
\href{https://goo.gl/KXpccP}{leftover hash lemma}.

First, the basic idea of the proof: For every \(m\times (n+1)\) matrix
\(A'\) over \(\Z_q\), define \(h_{A'}:\Z_q^m \rightarrow \Z_q^{n+1}\) to
be the map \(h_{A'}(w)=w^\top A'\). This collection can be shown to be a
...
and \(z\) is less than \(2^{-n}\). If not, the distance between
\((w^\top A',A')\) and \((z,A')\) would be at least
\(2^{-n}\cdot 2^{-n}>2^{-10n}\).

\textbf{Proof of \cref{Leftoverhashlem}:}\footnote{This is based on
  notes from
...
\textbf{Step 2:} The statistical difference between \((H(W),H)\) and
\((V,H)\) is less than \(\epsilon\). Denote the statistical difference
by \(\Delta((H(W),H),(V,H))\), then
...
This completes the proof of \cref{Leftoverhashlem} and hence the
theorem.

\end{proof}

\begin{pause} \label[pause]{11-The-proof-of-crefLWEEN}
...
\section{But what are lattices?}\label{11-But-what-are-lattices}
...
\begin{figure}
\centering
\includegraphics[width=\textwidth, height=0.25\paperheight, keepaspectratio]{../figure/Lattice-reduction.png}
