...
\begin{proof}[Proof of \cref{primesfirstlem}] \label[proof]{Let-n-be-a-number-and-let}
...
\begin{proof}[Proof of \cref{primesseclem}] \label[proof]{Let-np-cdots-pk---and-sup}
...
\begin{lemma}[Even one's] \label[lemma]{evenprob}
...
\begin{proof}[Proof of \cref{evenprob}] \label[proof]{Let-A---x-in-n--sumin--xi}

Let \(A = \{ x \in \{0,1\}^n : \sum_{i=0}^{n-1} x_i = 0 \mod 2 \}\).
Since every \(x\) is obtained with probability \(2^{-n}\), to show this
we need to show that \(|A|=2^n/2=2^{n-1}\). For every
\(x_0,\ldots,x_{n-2}\), if \(\sum_{i=0}^{n-2} x_i\) is even then
\((x_0,\ldots,x_{n-1},0)\in A\) and
\((x_0,\ldots,x_{n-1},1) \not\in A\). Similarly, if
\(\sum_{i=0}^{n-2} x_i\) is odd then \((x_0,\ldots,x_{n-1},1) \in A\)
and \((x_0,\ldots,x_{n-1},0)\not\in A\). Hence, for every one of the
\(2^{n-1}\) prefixes \((x_0,\ldots,x_{n-2})\), there is exactly a single
continuation of \((x_0,\ldots,x_{n-2})\) that places it in \(A\).
...
\(1/4\). (It is a great exercise for you to pause here and verify that
...
experiment. Formally, a random variable is simply a function
\(X:\{0,1\}^n \rightarrow \R\) that maps every outcome
\(x\in \{0,1\}^n\) to a real number \(X(x)\).\footnote{In many
  probability texts a random variable is always defined to have values
  in the set \(\R\) of real numbers, and this will be our default option
  as well. However, in some contexts in theoretical computer science we
  can consider random variables mapping to other sets such as
  \(\{0,1\}^*\).} For example, the function
\(sum:\{0,1\}^n \rightarrow \R\) that maps \(x\) to the sum of its
coordinates (i.e., to \(\sum_{i=0}^{n-1} x_i\)) is a random variable.
...
\begin{proof} \label[proof]{beginsplitE-XY--sumxin-n-}
...
\footnote{TODO: add exercise on simulating die tosses and choosing a
...
\begin{lemma}[Independant random variables] \label[lemma]{indcoins}
...
average income to be larger than 70,000. From this example you can
already see that in many situations, Markov's inequality will not be
